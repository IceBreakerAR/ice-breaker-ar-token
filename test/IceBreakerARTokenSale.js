...
const REFERRER_BONUS_RATE = new BigNumber(500);
...
const PRE_SALE_TOKEN_CAP = new BigNumber(57.5).mul(MILLION).mul(TO_TOKEN_DECIMALS);
const TOKEN_SALE_TOKEN_CAP = new BigNumber(212.5).mul(MILLION).mul(TO_TOKEN_DECIMALS);
const TOTAL_TOKENS_SUPPLY = new BigNumber(300).mul(MILLION).mul(TO_TOKEN_DECIMALS);


contract('IceBreakerARTokenSale', async function (accounts) {
  ...
  async function doBuy(tokenSaleInstance, acc, weiAmount, expectedRate, referrer) {
    ...
    const tokenInstance = IceBreakerARToken.at(await tokenSaleInstance.token());
    ...
    postTokensSold.sub(preTokensSold).should.be.bignumber.equal(tokensBought.add(tokensReferrerBonus));

    // check total supply is ok
    const postTotalSupply = await tokenInstance.totalSupply();
    postTotalSupply.sub(preTotalSupply).should.be.bignumber.equal(tokensBought.add(tokensReferrerBonus));
  ...
  describe('Contract tests', function () {
    it('Should reject deploying the Token Sale contract with no vault wallet', async function () {
      await IceBreakerARTokenSale.new(null, WALLET_AIRDROP, WALLET_KYC, TOKEN_RATE_BASE_RATE, REFERRER_BONUS_RATE, MAX_TX_GAS_PRICE).should.be.rejectedWith(EVMThrow);
    });

    it('Should reject deploying the Token Sale contract with no airdrop wallet', async function () {
      await IceBreakerARTokenSale.new(WALLET_VAULT, null, WALLET_KYC, TOKEN_RATE_BASE_RATE, REFERRER_BONUS_RATE, MAX_TX_GAS_PRICE).should.be.rejectedWith(EVMThrow);
    });

    it('Should reject deploying the Token Sale contract with no kyc wallet', async function () {
      await IceBreakerARTokenSale.new(WALLET_VAULT, WALLET_AIRDROP, null, TOKEN_RATE_BASE_RATE, REFERRER_BONUS_RATE, MAX_TX_GAS_PRICE).should.be.rejectedWith(EVMThrow);
    });

    it('Should reject deploying the Token Sale contract with no base rate', async function () {
      await IceBreakerARTokenSale.new(WALLET_VAULT, WALLET_AIRDROP, WALLET_KYC, 0, REFERRER_BONUS_RATE, MAX_TX_GAS_PRICE).should.be.rejectedWith(EVMThrow);
    });

    it('Should reject deploying the Token Sale contract with no referrer bonus rate', async function () {
      await IceBreakerARTokenSale.new(WALLET_VAULT, WALLET_AIRDROP, WALLET_KYC, TOKEN_RATE_BASE_RATE, 0, MAX_TX_GAS_PRICE).should.be.rejectedWith(EVMThrow);
    });

    it('Should reject deploying the Token Sale contract with no Max Tx Gas Price', async function () {
      await IceBreakerARTokenSale.new(WALLET_VAULT, WALLET_AIRDROP, WALLET_KYC, TOKEN_RATE_BASE_RATE, REFERRER_BONUS_RATE, 0).should.be.rejectedWith(EVMThrow);
    });

    it('Should deploy the Token Sale contract', async function () {
      tokenSaleInstance = await IceBreakerARTokenSale.new(WALLET_VAULT, WALLET_AIRDROP, WALLET_KYC, TOKEN_RATE_BASE_RATE, REFERRER_BONUS_RATE, MAX_TX_GAS_PRICE);
      tokenInstance = IceBreakerARToken.at(await tokenSaleInstance.token());
    ...
    it('Should reject reserving more tokens than pre sale cap', async function () {
      ...
      const tokensSold = await tokenSaleInstance.tokensSold();
      const tokensReserved = await tokenSaleInstance.tokensReserved();
      let amount = PRE_SALE_TOKEN_CAP.sub(tokensSold).sub(tokensReserved).add(1);
    ...
    it('Token should be owned by the Token Sale', async function () {
      ...
      assert.equal(owner, tokensaleAddress, "Owner of IceBreakerARToken should be IceBreakerARTokenSale");
    });

    it('Token should have mintingFinished be false', async function () {
      const mintingFinished = await tokenInstance.mintingFinished();

      assert.equal(mintingFinished, false, "IceBreakerARToken should not have finished minting");
    });
    ...
    it('Should reject buying less than minimum during presale', async function () {
      const amount = SALE_WEI_MIN_TX.sub(1);
    ...
    it('Should log purchase', async function () {
      const { logs } = await tokenSaleInstance.sendTransaction({ value: SALE_WEI_MIN_TX, from: WALLET_INVESTOR_1 });

      const event = logs.find(e => e.event === 'TokenPurchase');

      should.exist(event);
      event.args.purchaser.should.equal(WALLET_INVESTOR_1);
      event.args.value.should.be.bignumber.equal(SALE_WEI_MIN_TX);

      const expectedTokenAmount = TOKEN_RATE_20_PERCENT_BONUS.mul(SALE_WEI_MIN_TX);
      event.args.amount.should.be.bignumber.equal(expectedTokenAmount);
    });

    it('Should assign tokens to sender', async function () {
      let preBalance = await tokenInstance.balanceOf(WALLET_INVESTOR_1);
      await tokenSaleInstance.sendTransaction({ value: SALE_WEI_MIN_TX, from: WALLET_INVESTOR_1 });
    ...
    it('Should buy exactly the minimum', async function () {
      ...
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, SALE_WEI_MIN_TX, TOKEN_RATE_20_PERCENT_BONUS);
    });
    ...
    it('Should buy 2 ETH worth of tokens at (PreSale 20% Bonus)', async function () {
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, new BigNumber(2).mul(TO_WEI), TOKEN_RATE_20_PERCENT_BONUS);
    });

    it('Should buy 60 ETH worth of tokens at (PreSale 20% Bonus)', async function () {
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, new BigNumber(60).mul(TO_WEI), TOKEN_RATE_20_PERCENT_BONUS);
    });

    it('Should buy 80 ETH worth of tokens at (PreSale 20% Bonus)', async function () {
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, new BigNumber(80).mul(TO_WEI), TOKEN_RATE_20_PERCENT_BONUS);
    });
    ...
    it('Should buy 1200 ETH worth of tokens at PreSale (PreSale 20% Bonus)', async function () {
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, new BigNumber(1200).mul(TO_WEI), TOKEN_RATE_20_PERCENT_BONUS);
    });

    it('Should buy 1500 ETH worth of tokens at PreSale (PreSale 20% Bonus)', async function () {
      const tokens = await doBuy(tokenSaleInstance, WALLET_INVESTOR_1, new BigNumber(1500).mul(TO_WEI), TOKEN_RATE_20_PERCENT_BONUS);
    });
    ...
    it('Should reserve tokens during main sale', async function () {
      let user = WALLET_INVESTOR_5;
      let amount = new BigNumber(2).mul(TO_TOKEN_DECIMALS);
      ...
      const userReservedAmount = await tokenSaleInstance.getReservedAmount(user);
      userReservedAmount.should.be.bignumber.equal(new BigNumber(5).mul(TO_TOKEN_DECIMALS));

      const tokensReserved = await tokenSaleInstance.tokensReserved();
      tokensReserved.should.be.bignumber.equal(new BigNumber(5).mul(TO_TOKEN_DECIMALS));
    });
    ...
    it('Should reject reserving more tokens than Token Sale cap', async function () {
      let user = WALLET_INVESTOR_5;
      const tokensSold = await tokenSaleInstance.tokensSold();
      const tokensReserved = await tokenSaleInstance.tokensReserved();
      let amount = TOKEN_SALE_TOKEN_CAP.sub(tokensSold).sub(tokensReserved).add(1);
    ...
    it('Should call finishContract', async function () {
      ...
      const investor_5_ReservedAmount = await tokenSaleInstance.getReservedAmount(WALLET_INVESTOR_5);
      await tokenSaleInstance.cancelReservedTokens(WALLET_INVESTOR_5, investor_5_ReservedAmount, { from: WALLET_OWNER });
    ...
    it('Should reject minting 1 Token from the vault wallet (even though it owns the IceBreakerARToken contract now)', async function () {
      await tokenInstance.mint(WALLET_INVESTOR_1, 1, { from: WALLET_VAULT }).should.be.rejectedWith(EVMThrow);
    });
